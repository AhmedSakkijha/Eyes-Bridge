...
export async function startSpeechRecognition() {
  ...
  try {
    ...
    mediaRecorder.onstop = async () => {
      ...
      reader.onloadend = async () => {
        ...
        try {
          ...
          handlers.onError('Audio processing error: ' + (error instanceof Error ? error.message : String(error)));
        }
      };
    };
    ...
    handlers.onError('Speech recognition error: ' + (error instanceof Error ? error.message : String(error)));
...
export const handleCommand = async (
  ...
  try {
    ...
    switch (data.action) {
      ...
      case 'startNavigation':
        if (data.parameters && data.parameters.destination) {
          ...
          await speakText('Destination not specified.');
        }
        break;
      default:
    ...
    await speakText('An error occurred while processing the command');
  }
};
...
export async function startNavigation(destination: string): Promise<string> {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : 'An error occurred while starting navigation';
